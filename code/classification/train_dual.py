...
from FCdual import FusionModel
...
def train(bert_model, gene_model, model, gene_optimizer, bert_optimizer, optimizer, dataset_loader, train_phase, device):
    ...
    for i, data in enumerate(dataset_loader):
        ...
        Y_prob, Y_hat, Y_reg, Reg, features = model(pooler_output, gene_output)
        # --------------- for debugging --------------
        # print(Y_prob)
        ...
        class_label = (label != 0).float()  # 将非0的label设置为1，0的label保持为0
        class_loss = classification_criterion(Y_prob, class_label)
        reg_loss = regression_criterion(Reg, label.float())
        loss = class_loss + reg_loss
        
        mae = mean_absolute_error(Reg, label)  # 使用新的评估指标函数
        ...
        total_pred = torch.cat([total_pred.to(device),Reg],dim=0)
